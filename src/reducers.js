...
export function test_createReducer_scope (t) {
    ...
    t.deepEqual(reducer(10, actions.set(3)), 3)
    t.deepEqual(reducer(10, actions.baz.set(3)), 10, "sibling actions do not affect reducer")
    t.deepEqual(reducer(10, { type: "unknownAction" }), 10)

    const topReducer = createReducer(actions, {
        inc: (state) => state + 1,
        set: (_, { value }) => value,
    }, 0)
    ...
    t.deepEqual(topReducer(10, actions.foo.bar.set(3)), 10, "child actions do not affect reducer")
...
export function createReducer (actions, baseReducers, initState, scope = []) {
    for (const type in baseReducers) {
        if (!inScope(actions, type, scope)) { throw new Error(`Unknown action type: ${type}`) }
    }
